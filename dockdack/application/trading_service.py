...
class TradingService:
    ...
    def __init__(self, broker_factory: Callable[[Market], KiwoomBroker] | None = None,
        ...
        if self.mode in (TradingMode.REAL, TradingMode.DEMO):
            ...
            if self.mode is TradingMode.DEMO:
                from dockdack.environment_store import demo_scope
                self._storage_scope = demo_scope(self._storage_scope, os.environ.get("DOCKDACK_DEMO_GENERATION", "0"))
            if broker_factory is None:
