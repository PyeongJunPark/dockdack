...
class KiwoomBroker:
    ...
    def list_execution_history(
        ...
        *,
        ...
        if selected is Market.DOMESTIC and any(
                record.order_quantity > 1 and record.filled_quantity > 0 for record in records):
            # kt00007's cntr_uv is a unit price, NOT an authoritative average.
            # kt00009 documents individual cntr_no/cntr_qty/cntr_uv fills.
            # Finish both paginated reads before corroborating their totals.
            fills = []
            for page in self._http_for(selected).iter_pages(
                api_id="kt00009", path="/api/dostk/acnt",
                body={"ord_dt": day.strftime("%Y%m%d"), "stk_bond_tp": "1",
                      "mrkt_tp": "0", "sell_tp": "0", "qry_tp": "1",
                      "stk_cd": "", "fr_ord_no": "", "dmst_stex_tp": "%"},
                max_pages=max_pages,
            ):
                batch = page.body.get("acnt_ord_cntr_prst_array")
                if not isinstance(batch, list):
                    raise ValueError("개별 체결 내역 응답 목록이 없거나 잘못되었습니다.")
                fills.extend(batch)
            records = _domestic_weighted_fills(records, fills, day)
...
def _domestic_weighted_fills(records, raw_fills, day):
    """Corroborate dated cumulative snapshots with unique official kt00009 fills.

    Source: Kiwoom-Securities/Kiwoom-REST-API, examples/국내주식/계좌/
    get_domestic_account_order_fill_status.py. No aggregate account amount or
    last execution price is used as an order's average cost.
    """
    unique, groups = {}, {}
    for raw in raw_fills:
        if not isinstance(raw, dict):
            raise ValueError("개별 체결 내역 행이 올바르지 않습니다.")
        fill_id = raw.get("cntr_no")
        if (not isinstance(fill_id, str) or not fill_id.isascii()
                or not fill_id.isdigit() or int(fill_id) == 0):
            raise ValueError("개별 체결번호가 없어 평균 체결가를 확인할 수 없습니다.")
        record = _execution_history_record(
            {**raw, "ord_remnq": "0", "ori_ord": raw.get("orig_ord_no", "")},
            Market.DOMESTIC, day, "kt00009")
        fill_time = raw.get("cntr_tm")
        if not isinstance(fill_time, str):
            raise ValueError("개별 체결시각을 확인할 수 없습니다.")
        try:
            datetime.strptime(fill_time, "%H:%M:%S")
        except ValueError:
            raise ValueError("개별 체결시각이 올바르지 않습니다.") from None
        if record.filled_quantity <= 0 or record.reported_fill_price is None:
            raise ValueError("개별 체결수량/단가가 올바르지 않습니다.")
        order_key = record.order_number.lstrip("0")
        key = order_key, record.exchange, fill_id.lstrip("0")
        event = (record.symbol, record.side, record.order_quantity,
                 record.filled_quantity, record.reported_fill_price,
                 record.original_order_number.lstrip("0"), fill_time)
        if key in unique:
            if unique[key] != event:
                raise ValueError("동일 체결번호의 내역이 서로 달라 평균가를 확인할 수 없습니다.")
            continue
        unique[key] = event
        groups.setdefault(order_key, []).append((record, fill_time))
    result = []
    for snapshot in records:
        events = groups.get(snapshot.order_number.lstrip("0"), ())
        if snapshot.order_quantity <= 1 or snapshot.filled_quantity <= 0 or not events:
            result.append(snapshot)
            continue
        identity = (snapshot.symbol, snapshot.exchange, snapshot.side, snapshot.order_quantity)
        if any((row.symbol, row.exchange, row.side, row.order_quantity) != identity
               for row, _ in events):
            raise ValueError("주문 누적 내역과 개별 체결의 종목·거래소·방향·수량이 다릅니다.")
        # Corrections/cancellations can chain orders. Do not infer attribution.
        if snapshot.original_order_number.strip("0 ") or any(
                row.original_order_number.strip("0 ") for row, _ in events):
            result.append(snapshot)
            continue
        total = sum((row.filled_quantity for row, _ in events), Decimal(0))
        if total != snapshot.filled_quantity:
            # The account may fill between the two reads. Leave cost unknown;
            # a future complete refresh can establish a matching snapshot.
            result.append(snapshot)
            continue
        amount = sum((row.filled_quantity * row.reported_fill_price
                      for row, _ in events), Decimal(0))
        result.append(replace(snapshot, fill_price=amount / total, fill_amount=amount,
                              price_basis="weighted_fills", source_api="kt00007+kt00009",
                              fill_time=max(stamp for _, stamp in events)))
    return result
