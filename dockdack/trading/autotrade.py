...
class AutoTrader:
    ...
    def _lot_cash_account(self, account, rule):
        ...
        if rule.side is not OrderSide.BUY or self._prototype_source_for(rule) is None:
            return account
        reserved = Decimal(0)
        for row in self.store.order_history(limit=None, market=account.market, side="buy", statuses=("accepted", "submitting", "unknown")):
    ...
    def _ensure_environment(self, instrument=None, *, orders=False):
        ...
        try:
            ...
            scope = getattr(self.service, "storage_scope", "demo")
            if self._mode is TradingMode.DEMO and scope != "demo":
                if scope == "unconfigured" or scope != self.store.storage_scope:
                    raise ValueError("모의 API 키/계정 세대와 매매 기록 범위가 다릅니다. 기존 장부의 명시적 이관이 필요합니다.")
        ...
        try:
            ...
            for _ in range(len(items) + 500):
                ...
                if (self.enable_holdings_exits and
                        (entry_checks >= self.holdings_recheck_items or
                         (self.clock() - last_exit_check).total_seconds() >= self.holdings_recheck_seconds)):
                    self._holdings_pass(sent, checkpoint=priority_checkpoint, progress=progress)
                    last_exit_check, entry_checks = self.clock(), 0
                self._read_external()
            ...
            priority_checkpoint()
            if self.enable_holdings_exits and not self._stop.is_set() and entry_checks:
                self._holdings_pass(sent, checkpoint=priority_checkpoint, progress=progress)
            return results
        except Exception as exc:
