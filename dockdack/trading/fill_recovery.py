...
class FillRecovery:
    ...
    def _check_environment(self, *, allow_unconfigured=False):
        ...
        if self._mode is TradingMode.REAL or scope not in {"demo", "unconfigured", None, ""} or self._storage_scope not in {"demo", "unconfigured", None, ""}:
            if scope != self._storage_scope or scope != getattr(self.store, "storage_scope", "unconfigured"):
                raise _EnvironmentMismatch("체결가 보완 서비스와 매매 기록의 계좌 키 범위가 다릅니다.")
    ...
    def _recover(self, row, records, local_index):
        ...
        broker_average = (record.market is Market.US and record.source_api == "ust21180" and record.price_basis == "broker_average"
        ...
        weighted_fills = (record.market is Market.DOMESTIC and record.source_api == "kt00007+kt00009"
                          and record.price_basis == "weighted_fills"
                          and _positive(record.filled_quantity) and _positive(record.fill_amount)
                          and record.fill_price == record.fill_amount / record.filled_quantity)
        verified_average = broker_average or weighted_fills
        usable = (single_share or verified_average) and isinstance(record.fill_price, Decimal) and _positive(record.fill_price)
        if usable and verified_average:
        ...
        message = (f"주문번호 {row['order_number']} · {record.filled_quantity}주 "
                   f"{'평균 체결가' if verified_average else '체결가'} {record.fill_price} {record.currency} 확인"
