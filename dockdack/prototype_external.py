...
class PrototypeWorker:
    ...
    def __init__(self, model_id, *, bundle_root=None, state_path=None, predictors=None):
        ...
        self.bundle_root = Path(bundle_root or ROOT / self.bridge_type.bundle_directory)
...
class PrototypeProcessClient:
    ...
    def _start(self):
        if self.is_alive:
            return
        self.close()
        command = [sys.executable, "-u", "-X", "utf8", "-m", "examples.run_prototype_signal",
        ...
        if self.state_path is not None:
            ...
            self._stderr = log.open("a", encoding="utf-8")
        self._responses = queue.Queue()
        self.process = subprocess.Popen(
            command, cwd=ROOT, env=child_environment(), stdin=subprocess.PIPE,
            stdout=subprocess.PIPE, stderr=self._stderr or subprocess.DEVNULL,
            text=True, encoding="utf-8", bufsize=1,
            creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
        stream, responses = self.process.stdout, self._responses
        def read_responses():
            try:
                while True:
    ...
    def request(self, operation, *, start=True, **values):
        with self._lock:
            ...
            try:
                ...
                self.close()
                raise TimeoutError("External prototype response timed out; this feed is blocked") from exc
            except Exception:
                self.close()
                raise
