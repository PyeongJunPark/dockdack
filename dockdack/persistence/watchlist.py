...
class WatchStore:
    ...
    def __init__(self, path: Path | str, *, seed_defaults: bool = False, mode: TradingMode = TradingMode.DEMO, storage_scope: str | None = None):
        ...
        with self.connection() as db:
            ...
            if 'app_environment' in tables:
                ...
                if tables and (self._mode is TradingMode.REAL or self._storage_scope != "demo"):
                    raise ValueError("환경 정보가 없는 기존 기록을 현재 계정에 자동 귀속하지 않습니다. 명시적 복사 이관이 필요합니다.")
        ...
        with self.connection() as db:
            ...
            db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS close_lot_allocations (
                    rule_id TEXT NOT NULL REFERENCES rules(id), lot_id TEXT NOT NULL REFERENCES attempts(rule_id),
                    quantity TEXT NOT NULL, fill_offset TEXT NOT NULL, created_at TEXT NOT NULL,
                    buy_filled_quantity TEXT, buy_average_price TEXT,
                    PRIMARY KEY(rule_id,lot_id));
                CREATE INDEX IF NOT EXISTS close_sells_by_lot ON close_lot_allocations(lot_id);
            ...
            for table_name in ("attempts", "order_execution_snapshots", "order_fill_recovery", "rules", "watchlist", "prototype_sell_allocations", "close_lot_allocations"):
    ...
    @staticmethod
    def _prototype_allocations(db, watch_id=None, rule_id=None):
        # Filter before materialising either branch. Unrelated ledger history
        # must not make one symbol's read proportional to the whole account.
        filters, values = [], []
        if watch_id is not None:
            filters.append("r.watch_id=?")
            values.append(watch_id)
        if rule_id is not None:
            filters.append("r.id=?")
            values.append(rule_id)
        suffix = (" AND " + " AND ".join(filters)) if filters else ""
        sql = """SELECT p.rule_id,p.lot_id,p.quantity,p.created_at,p.buy_filled_quantity,p.buy_average_price,
                ...
                WHERE NOT EXISTS(SELECT 1 FROM prototype_sell_allocations own WHERE own.rule_id=retry.rule_id)""" + suffix
        rows = tuple(dict(row) for row in db.execute(sql, values + values))
        bulk = db.execute("""SELECT p.rule_id,p.lot_id,p.quantity,p.created_at,p.buy_filled_quantity,p.buy_average_price,
                         r.watch_id,r.side,r.status AS rule_status,1 AS bulk,p.fill_offset,r.quantity AS order_quantity
                         FROM close_lot_allocations p JOIN rules r ON r.id=p.rule_id WHERE 1=1""" + suffix, values)
        return rows + tuple(dict(row) for row in bulk)

    def _prototype_inventory(self, db, watch_id=None):
        from dockdack.strategy_lots import project_prototype_inventory
        rows = self._order_history(db, None, watch_id=watch_id)
        allocations = self._prototype_allocations(db, watch_id=watch_id)
    ...
    def prototype_pending_buys(self, watch_id, strategy_id=None):
        ...
        result = []
        for row in self.order_history(limit=None, watch_id=watch_id, statuses=PENDING, side="buy"):
    ...
    def prototype_sell_allocation(self, rule_id):
        with self.connection() as db:
            # Account-wide closing sales have several allocations and are not
            # a normal one-lot model exit or a model-origin BUY.
            allocation = next((row for row in self._prototype_allocations(db, rule_id=rule_id)
                               if not row.get("bulk")), None)
    ...
    def reserve_prototype_sell(self, rule_id, lot_id, quantity, *, now=None):
        ...
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            previous = next(iter(self._prototype_allocations(db, rule_id=rule_id)), None)
            if previous:
    ...
    def _prototype_claim_allowed(self, db, rule):
        ...
        if inventory["issues"]:
            return False
        allocations = {row["rule_id"]: row for row in self._prototype_allocations(db, watch_id=rule.watch_id)}
        ...
        for pending in self._order_history(db, None, watch_id=rule.watch_id, statuses=PENDING):
    ...
    def order_history(self, limit=500, *, watch_id=None, statuses=None, side=None, market=None) -> tuple[dict, ...]:
        ...
        with self.connection() as db:
            return self._order_history(db, limit, watch_id=watch_id, statuses=statuses, side=side, market=market)

    @staticmethod
    def _order_history(db, limit, *, watch_id=None, statuses=None, side=None, market=None):
        sql = """
        ...
        """
        clauses, parameters = [], []
        for column, value in (("a.watch_id", watch_id), ("r.side", side), ("w.market", market)):
            if value is not None:
                clauses.append(column + "=?")
                parameters.append(getattr(value, "value", value))
        if statuses is not None:
            statuses = tuple(statuses)
            if not statuses:
                return ()
        ...
        for row in db.execute(sql, parameters):
            item = dict(row)
            item["close_allocations"] = tuple(dict(allocation) for allocation in db.execute(
                "SELECT lot_id,quantity,fill_offset FROM close_lot_allocations WHERE rule_id=? ORDER BY CAST(fill_offset AS NUMERIC)",
                (item["rule_id"],))) if str(item["rule_id"]).startswith("close-") else ()
    ...
    def cached_snapshots(self, items):
        """One consistent batched read, isolating corrupt per-symbol cache rows."""
        ...
        with self.connection() as db:
            ...
            for start in range(0, len(keys), 400):
                ...
                rows = db.execute("SELECT watch_id,data FROM snapshots WHERE watch_id IN ("
                                  + ",".join("?" for _ in batch) + ")", batch)
                for row in rows:
                    try:
                        result[row["watch_id"]] = self.decode_snapshot(selected[row["watch_id"]], row["data"])
                    except Exception as exc:
                        result[row["watch_id"]] = exc
        return result
