...
from PySide6.QtWidgets import QComboBox, QFrame, QGridLayout, QHBoxLayout, QLineEdit, QPushButton, QVBoxLayout, QWidget
...
def connection_view(status: dict) -> dict:
    ...
    environment = {"demo": "모의주문 · 가상 자금", "real": "실전주문 · 실제 자금"}.get(
        status.get("trading_mode"), "거래 환경 미확인 · 주문 환경을 확인하세요")
    if status.get("orders_enabled"):
        gate = "ON · 유효 신호 + 정규장 + 잔고·수량·금액 검사 통과 시에만 " + environment
    elif status.get("pending_arm"):
        gate = "OFF · 전체 조회 검증 후 ON 예약됨 · OFF 버튼으로 예약 취소 가능 · " + environment
    else:
        gate = "OFF · 새 자동주문 차단 / 감시와 신호 수신은 별도 동작 · " + environment
...
class SignalConnectionPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.source_selector.setAccessibleName('상태와 파일을 검사할 신호 출처')
        self.source_selector.currentIndexChanged.connect(
            lambda _: self.request_source.emit(str(self.source_selector.currentData() or '')))
    ...
    def set_status(self, status: dict):
        choices = tuple((row.get('label') or row['source_id'], row['source_id'])
                        for row in status.get('source_options', ()))
        selected = status.get('source_id')
        self.source_selector.blockSignals(True)
        try:
            if choices != getattr(self, '_source_choices', None):
                self._source_choices = choices
                self.source_selector.clear()
                for title, key in choices:
                    self.source_selector.addItem(title, key)
            self.source_selector.setCurrentIndex(self.source_selector.findData(selected))
            self.source_selector.setVisible(len(choices) > 1)
        finally:
