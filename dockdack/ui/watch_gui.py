...
from dockdack.environment_store import environment_base, selected_mode, scoped_store_path, store_for_service
...
from dockdack.ui.workers import ActivityReadWorker, WorkspaceReadWorker
...
class WatchlistDialog(QDialog):
    def __init__(self, service: TradingService, store: WatchStore | None = None, parent=None, *, defer_workspace=False, session_lock=None):
        super().__init__(parent)
        ...
        if (self.store.mode is TradingMode.REAL or scope not in (None, 'demo')) and self.store.storage_scope != scope:
            raise ValueError('API 인증 범위와 저장소가 다릅니다. 기존 장부는 명시적으로 이관해야 합니다.')
        self._defer_workspace = defer_workspace
        self._workspace_worker = None
        self._workspace_pending = False
        self._workspace_minimum_days = 0
        ...
        self._pending_session_lock = None
        ...
        self._selected_connection_source = None
        ...
        if self._defer_workspace:
            self.request_workspace_reload()
        else:
            items = self.store.items()
            self._apply_cached_snapshots(self.store.cached_snapshots(items))
            self.reload_tables(items=items)
        ...
        if screen is not None:
            ...
            self.resize(min(1360, max(self.minimumWidth(), available.width() - 24)),
                        min(900, max(self.minimumHeight(), available.height() - 48)))
    ...
    def request_environment(self, mode):
        ...
        try:
            ...
            target = candidate_store.path if candidate_store is not None else scoped_store_path(
                candidate, base_folder=self._environment_base)
            candidate_lock = self._acquire_environment_lock(target)
            ...
            self._pending_session_lock = candidate_lock
            ...
            if candidate_lock is not None and candidate_lock is not self._session_lock:
                candidate_lock.release()
            if candidate is not None and callable(getattr(candidate, 'revoke_live_risk', None)):
                candidate.revoke_live_risk()
    ...
    def _acquire_environment_lock(self, store_path):
        path = (Path(store_path).parent / 'session.lock').resolve()
        if self._session_lock is not None and self._session_lock.path.resolve() == path:
            return self._session_lock
        lock = SessionLock(path)
        lock.acquire()  # Must succeed before opening or migrating the target DB.
        return lock

    def _cancel_pending_environment(self):
        pending, self._pending_environment = self._pending_environment, None
        lock, self._pending_session_lock = self._pending_session_lock, None
        if lock is not None and lock is not self._session_lock:
            lock.release()
        if pending and callable(getattr(pending[0], 'revoke_live_risk', None)):
            pending[0].revoke_live_risk()
        if hasattr(self, 'environment_timer'):
    ...
    def _finish_environment_switch(self):
        if (not self._pending_environment or self.worker is not None or self._inspection_worker is not None
                or self._activity_worker is not None or self._schedule_probe is not None or self._workspace_worker is not None):
            return
        ...
        self._session_lock, self._pending_session_lock = self._pending_session_lock, None
        self._pending_environment = None
        self.environment_timer.stop()
        self.service, self.store = service, store
        self.engine = engine
        if previous_lock is not None and previous_lock is not self._session_lock:
            previous_lock.release()
        self._market_open = {}
        self._market_display_open = {}
        self._market_status_minute = None
        self._notification_initialized = False
        self._ledger_collector = collector
        self._schedule_minute = None
        self.scheduler = scheduler
        self.portfolio = portfolio
        self._portfolio_payload = self.portfolio.snapshot()
        self.fill_recovery = recovery
        self._fill_recovery_status = self.fill_recovery.status()
        ...
        self.model_performance_panel.set_context(store)
    ...
    def _build(self):
        ...
        brand = QVBoxLayout()
        brand.setSpacing(2)
        brand.addWidget(label(f'{APP_NAME}  ver {APP_RELEASE}', "heading"))
        ...
        self.trade_journal_panel.request_refresh.connect(self.refresh_journal)
        self.model_performance_panel = ModelPerformancePanel(self.store)
        self.model_performance_panel.request_refresh.connect(self.refresh_model_performance)
        ...
        self.signal_connection_page = QTabWidget()
        status_scroll = QScrollArea()
        status_scroll.setWidgetResizable(True)
        status_scroll.setFrameShape(QFrame.Shape.NoFrame)
        status_scroll.setWidget(self.signal_connection_panel)
        self.signal_connection_page.addTab(status_scroll, '상태·검사')
        self.tabs.removeTab(self.tabs.indexOf(self.external_panel))
        self.signal_connection_page.addTab(self.external_panel, '모델·연결 설정')
    ...
    def update_controls(self):
        busy = self.worker is not None or self._workspace_worker is not None
    ...
    def _sync_order_controls(self, status=None):
        ...
        display_key = (armed, self.pending_auto_arm, monitoring, busy, self._confirming_orders,
                       ...
                       self._workspace_worker is not None, self._workspace_error)
    ...
    def connection_status(self):
        ...
        options = self.connection_source_options(config)
        selected = next((row for row in options if row['source_id'] == self._selected_connection_source),
                        options[0] if options else {'source_id': source, 'input_path': inbox})
        source, inbox = selected['source_id'], selected['input_path']
        selected_entry = getattr(self.engine, 'external_sources', {}).get(source) if config == applied else None
        reader = selected_entry[1] if selected_entry else (
            self.engine.external_reader if source == config[2] and config == applied else None)
        random = random and source == config[2]
        active = bool(self.monitoring and self.engine.external_only and reader)
        blocked = getattr(self.engine, 'external_source_errors', {}).get(source, '') or self._last_export_error or self._last_worker_error
        if not blocked and source == config[2]:
            blocked = self.engine.external_error
        ...
        return {"configured": external, "active": active, "producer": "random-demo" if random else "external-file" if external else "manual",
                "source_id": source, "input_path": inbox, "output_path": outbox, "source_options": options,
                ...
                "block_reason": blocked, "inspection": self._inspection if self._inspection_config == config and self._inspection_source in (None, source) else None,
    ...
    def connection_source_options(self, config):
        options = [{'source_id': config[2], 'input_path': config[3], 'label': config[2]}]
        for source, path in config[11]:
            if source and path:
                options.append({'source_id': source, 'input_path': path, 'label': source})
        return options

    def select_connection_source(self, source):
        self._selected_connection_source = source or None
        self._update_connection()
    ...
    def open_connection_settings(self):
        ...
        self.signal_connection_page.setCurrentWidget(self.external_panel)
    ...
    def open_connection_folder(self):
        status = self.connection_status()
        folder = Path(status['input_path'] or status['output_path']).expanduser().resolve().parent
    ...
    def inspect_signals(self):
        ...
        _, random, primary_source, _, _, quantity, krw, usd, _ = config[:9]
        source, inbox = status['source_id'], status['input_path']
        random = random and source == primary_source
        try:
            ...
            return
        mode = selected_mode(self.service)
        worker = Worker(lambda: inspect_signal_file(inbox, policy, mode=mode))
    ...
    def _inspection_completed(self, result, error):
        ...
        if self._close_when_idle:
            ...
            return
        config = self._applied_external_config if self.monitoring and self._applied_external_config else self._external_config_key()
        if self._inspection_config != config or self._inspection_source != self.connection_status()['source_id']:
            self._inspection = None
            self._update_connection()
    ...
    def _build_external_tab(self):
        ...
        self.read_signals_button = QPushButton("선택 출처 수신 시험 · 후보 등록")
        self.read_signals_button.setToolTip('연결 상태에서 선택한 출처를 한 번 접수합니다. 파일 검사와 달리 주문후보를 등록하며, 이후 자동주문 ON 시 사용될 수 있습니다.')
    ...
    def read_signals(self):
        ...
        try:
            ...
            entry = self.engine.external_sources.get(source)
            if entry is not None:
                policy, reader = entry
            else:
                policy = ExternalPolicy(source, policy.max_quantity, policy.max_krw, policy.max_usd,
                                        allow_market=policy.allow_market and source == policy.source_id)
                reader = SignalFileReader(self.store, status['input_path'], policy, self.engine.clock)
            def read():
                reader()
                return {}
    ...
    def _apply_cached_snapshots(self, cached):
        for key, value in cached.items():
            if isinstance(value, Exception):
                self.errors[key] = f'저장 데이터 오류: {value}'
            elif key not in self.fresh_ids:
                self.snapshots[key] = value

    def request_workspace_reload(self, *, minimum_days=0):
        """Serialize local metadata/cache work away from Qt, including model setup."""
        if self._close_when_idle:
            return
        self._workspace_minimum_days = max(self._workspace_minimum_days, minimum_days)
        if self._workspace_worker is not None:
            self._workspace_pending = True
            return
        store, minimum = self.store, self._workspace_minimum_days
        self._workspace_minimum_days = 0
        worker = WorkspaceReadWorker(store, minimum_days=minimum)
        self._workspace_worker = worker
        worker.signals.completed.connect(self._workspace_loaded)
        self.activity_pool.start(worker)
        self.update_controls()

    @Slot(object, object)
    def _workspace_loaded(self, result, error):
        self._workspace_worker = None
        if error is not None:
            self._workspace_error = f'관심종목 DB 읽기 실패 · 자동주문 OFF: {error}'
            self.engine.disarm()
    ...
    def _reload_activity(self, *, force=False, visible_force=False):
        ...
        ledger_needed = force or page in (self.order_history_panel, self.trade_journal_panel,
                                         self.model_performance_panel)
        worker = ActivityReadWorker(store, collector, categories=categories, previous_heads=heads,
                                    ledger_needed=ledger_needed, log_reader=collect_event_logs)
        self._activity_worker = worker
        worker.signals.completed.connect(self._activity_completed)
        self.activity_pool.start(worker)

    def refresh_journal(self):
        """A local refresh request shares the serialized ledger worker."""
        self.trade_journal_panel.refresh_button.setEnabled(False)
        self.trade_journal_panel.refresh_button.setText('일지 읽는 중…')
        self._reload_activity(force=True)

    def refresh_model_performance(self):
        """Read saved fills on the shared worker; never refresh broker accounts."""
        self.model_performance_panel.refresh_button.setEnabled(False)
        self.model_performance_panel.refresh_button.setText('성과 읽는 중…')
        self._reload_activity(force=True)
    ...
    def _activity_completed(self, result, error):
        ...
        if error is not None:
            ...
            if snapshot is not None:
                ...
                self.model_performance_panel.apply_snapshot(snapshot)
        pending, self._activity_pending = self._activity_pending, False
        self.trade_journal_panel.refresh_button.setEnabled(True)
        self.trade_journal_panel.refresh_button.setText('일지 새로고침')
        self.model_performance_panel.refresh_button.setEnabled(True)
        self.model_performance_panel.refresh_button.setText('성과 새로고침')
        self.model_performance_panel.refresh_button.setToolTip(
            self._last_log_error if error is not None else '저장된 체결 기록만 다시 계산합니다. API 조회나 주문을 하지 않습니다.')
        self.model_performance_panel.set_refresh_error(self._last_log_error if error is not None else '')
        if error is not None:
            self.trade_journal_panel.refresh_button.setToolTip(self._last_log_error)
        else:
            self.trade_journal_panel.refresh_button.setToolTip('저장된 주문 장부만 다시 읽습니다. API 조회나 매수·매도는 하지 않습니다.')
        if self._close_when_idle:
            QTimer.singleShot(0, self.close)
        elif self._pending_environment:
            self._finish_environment_switch()
    ...
    def _run(self, operation, *, streaming=False, done=None, focus_result=False, job_kind="task"):
        ...
        self.sweep_progress.set_activity('계좌·체결 조회 준비 · 보유 매도 우선 점검 → 관심종목 조회'
    ...
    def start_monitoring(self):
        if self._pending_environment or self._confirming_environment or self._workspace_worker is not None or self._workspace_error:
            return
    ...
    def enable_auto_orders(self):
        if (self.engine.orders_enabled or self.pending_auto_arm or self._confirming_orders or self._pending_environment
    ...
    def closeEvent(self, event):
        ...
        self.stop_monitoring()
        if self.worker or self._inspection_worker or self._activity_worker or self._schedule_probe or self._workspace_worker:
            event.ignore()
        else:
