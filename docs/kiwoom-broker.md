...
신규·취소 주문은 성공 코드(`return_code=0`)와 비어 있지 않은 문자열 주문번호가 모두 있어야
접수 성공으로 반환한다. 빈 응답이나 필수 값 누락은 `OrderOutcomeUnknown` 예외로 구분한다.
이는 주문 거절이나 미접수를 뜻하지 않으므로, 다시 주문하기 전에 주문·체결 내역을 확인해야 한다.
신규·취소 주문은 인증 오류가 발생해도 자동 재전송하지 않는다.
HTTP 리다이렉트도 자동으로 따라가지 않으며, 2xx 이외 응답은 성공으로 처리하지 않는다.
