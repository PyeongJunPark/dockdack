...
> 두 prototype 연결과 모델별 보유의 상세는 [모델 사용 안내](MARK1_1_PROTOTYPE.md)를 참고한다. [ver 0.0 안내](VER_0_0.md)는 과거 기능·검증 이력이다.

기본은 모의투자다. 실전은 [환경 선택과 안전장치](trading-modes.md)를 거친 별도 환경에서 사용한다.
신호를 만드는 모델/전략은 별도 코드에서 실행하고, DockDack은 **JSON 데이터만** 읽는다.
JSON 파일의 내용으로 임의 외부 코드를 로드하거나 실행하지 않는다. 제공된 두 prototype은 사용자가 연결하면 별도 프로세스로 실행하고, 임의 코드가 아닌 데이터 신호만 주고받는다. 신호 수신과 자동주문 활성화는 별도이며 기본은 OFF다.
...
5. 상단 **신호 연결**에서 출처 `source_id`, 입력/출력 경로, 주문당 최대 수량과 KRW/USD 상한을 확인한다.
   추가 신호기는 각각 다른 출처와 입력 파일로 연결한다. 기본으로 선택되는 두 prototype 외부 연결은 각각의 체크박스로 끌 수 있다. 내장 LSTM은 기본 선택되지 않으며, 호환 설정은 **기타·고급 설정 → 신호 방식·구형 모델 → 기존 내장 트리거**에 있다.
...
6. **감시 시작 (조회만)**을 누른다. 모델을 선택하면 외부 신호 모드가 적용되며 수동 가격/SMA 규칙은 실행하지 않는다. 별도 모드 전환은 **기타·고급 설정 → 신호 방식·구형 모델**에서 명시적으로 한다. 두 모델을 모두 꺼도 수동 규칙 모드로 자동 전환되지 않는다.
...
일반 외부 신호는 `take_profit_price`와 `stop_loss_price`를 함께 받아 접수된 매수에 저장한다. 두 prototype은 이 출처 기록과 별도로 **확인된 체결**을 원매수 건별로 대조하고, 모델별 실제 원가와 수량으로 청산한다. 접수만으로 체결이나 보유량을 만들지 않는다.
...
현재 통합 데스크의 기본 모델 연결은 `mark1 prototype`과 `mark1.1 prototype` 두 외부 신호기이며, 내장 LSTM30은 **기타·고급 설정 → 신호 방식·구형 모델 → 기존 내장 트리거**에서 명시적으로 선택하는 호환 기능이다. 선택 가능한 `RandomDemoSignals`는 고급 영역의 **모의 테스트 신호기**로, prototype AI 연결과 함께 선택하지 않는다.
