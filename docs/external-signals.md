...
> 현재 두 prototype 연결과 모델별 보유는 [최신 사용 안내](MARK1_1_PROTOTYPE.md), 공통 기능은 [ver 0.0 안내](VER_0_0.md)를 참고한다.
...
5. 상단 **외부 신호 연결 → 연결 설정**에서 출처 `source_id`, 입력/출력 경로, 주문당 최대 수량과 KRW/USD 상한을 입력한다.
   추가 신호기는 각각 다른 출처와 입력 파일로 연결한다. **기존 내장 트리거** 드롭다운을 `내장 트리거 없음 · 외부 신호만 사용`으로 두면 내장 LSTM을 사용하지 않는다. 기본으로 선택되는 두 prototype 외부 연결도 각각의 체크박스로 끌 수 있다.
...
현재 통합 데스크의 기본 모델 연결은 `mark1 prototype`과 `mark1.1 prototype` 두 외부 신호기이며, 내장 LSTM30은 **기존 내장 트리거** 드롭다운에서 명시적으로 선택하는 호환 기능이다. 선택 가능한 `RandomDemoSignals`는 별도 실행 없이 연결되는 **모의 테스트 전략**으로, prototype AI 연결과 함께 선택하지 않는다.
