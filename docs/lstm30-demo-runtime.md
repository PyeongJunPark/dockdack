...
기존 자동매매 대시보드에 LSTM 신호를 연결하려면 `LSTM30Demo.vbs`를 실행한다. 이 바로가기는 **자동주문 OFF**로 열리며, 화면의 ON 버튼에서 확인한 뒤 초기 전체 조회가 성공해야 주문이 켜진다. 기본 `DockDack.vbs`는 기존 트레이딩 데스크를 여는 용도로 유지한다.
