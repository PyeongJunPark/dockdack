...
> 이 문서는 이전 전용 실행기·명령행 호환 경로의 설명이다. 현재 기본 앱은
> `DockDack.vbs` 하나로 여는 **0.1(MK1) 통합 데스크**다. 현재 기능·사용 조건은 [기능서](FEATURES.md)를 따른다.
> 내장 LSTM은 일반 화면의 기본 모델이 아니라 **기타·고급 설정 → 신호 방식·구형 모델**의 호환 기능이다.
> 혼동을 주던 `LSTM30Demo.vbs` 바로가기는 제거했다. 아래 전용 실행기 옵션은 기본 GUI 설정이 아니다.

학습된 국내·미국 LSTM을 기존 `AutoTrader`와 연결하는 별도 백그라운드 실행기다. **모의투자만 지원**하며 실전 환경·실전 승인값을 받지 않는다. 코드 가져오기, 기본 실행, `--once`는 주문을 켜지 않는다.

## GUI에서 직접 자동주문 제어

현재 GUI 진입점은 **`DockDack.vbs` 하나**다. 오래된 `LSTM30Demo.vbs`는 제거했다. 기존 LSTM은 통합 GUI의 **기타·고급 설정 → 신호 방식·구형 모델**에서 내장 mark0로 선택하며, 기본 prototype 둘은 [외부 AI 신호기](MARK1_1_PROTOTYPE.md)로 연결한다. 현재 일반 화면의 기준은 [0.1(MK1) 기능서](FEATURES.md)다. 아래 전용 실행 명령과 저장 경로·한도는 이전 연구/진단용이며 현재 기본 자동 실행 경로의 설정이 아니다.
...
전용 GUI에는 **실제로 주문하는 엔진의 ON/OFF·감시 상태·신호 연결·주문 기록**이 표시된다. 내장 랜덤 신호가 아닌 `lstm30-mark0` 모델 신호이며, 실전 전환과 임의 전략 변경은 이 전용 실행기에서 막는다. 위 호환 명령은 미국 일반기업 보통주 상위 100종목을 감시하고 국내는 기존 삼성전자 1종목을 유지한다. 국내도 TOP100으로 확장하려면 별도로 `--top-domestic100`을 명시한다. 순위 옵션 없는 직접 실행은 기존 삼성전자/AAPL 연결 점검용이다.
